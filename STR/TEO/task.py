...
class cyclicScheduler:
    ...
    def sec(self):
        ...
        if(maxCi > minTi):
            print("ERROR: the maxCi is greater than minTi")
            return -1
...
class taskRT:
    ...
    """
    def __init__(self, num, time, per):
        self.num = num
        self.c = time
        self.period = per
    def __str__(self):
        return "task number {} with computing time {} and deadline {}".format(self.num, self.c, self.period)
...
def calcCyclicScheduler(c):
    ...
    print("=====> The tasks can be planable in a cyclic scheduler.")

c = cyclicScheduler('tasks3.txt')
